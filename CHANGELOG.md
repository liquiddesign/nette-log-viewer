...
### Fixed
- `LogViewerPresenter::formatTemplateFiles()` read the parent's private `$view` property (worked only through SmartObject magic, PHPStan `property.private` error with nette/application 3.3); uses `getView()` now.
